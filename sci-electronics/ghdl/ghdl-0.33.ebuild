...
SRC_URI="https://github.com/tgingold/${PN}/archive/v${PV}.tar.gz
	mirror://gnu/gcc/releases/gcc-${GCC_VERSION}/gcc-${GCC_VERSION}.tar.bz2"
LICENSE="GPL-2"
SLOT="0"
KEYWORDS="~amd64 ~ppc ~x86"
IUSE=""
DEPEND="dev-lang/gnat-gcc:${GNATGCC_SLOT}
	dev-libs/gmp
	dev-libs/mpfr
	dev-libs/mpc"
RDEPEND=""
...
src_prepare() {
	sed -i -e 's/ADAC = \$(CC)/ADAC = gnatgcc/' ${WORKDIR}/${P}/Makefile.in || die "sed failed"
#	sed -i \
...
#		gcc/vhdl/Make-lang.in || die "sed failed"
	...
	cd ${WORKDIR}/${P}
	./configure --with-gcc=${S} || die "GCC location configure failed"
	make copy-sources || die "Failed to copy sources"
}
...
src_compile() {
	PATH="${GNATGCC_PATH}:${PATH}" emake || die "Compilation failed"
}
